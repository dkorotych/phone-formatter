        ...
        if (StringUtils.hasText(phoneNumber)) {
            ...
            response.setNumbers(numbers.entrySet().stream().
                    map(entry -> {
                        ...
                        final String e164 = PHONE_NUMBER_UTIL.format(number, E164);
                        final String rfc3966 = PHONE_NUMBER_UTIL.format(number, RFC3966);
                        final String national = PHONE_NUMBER_UTIL.format(number, NATIONAL);
                        final String international = PHONE_NUMBER_UTIL.format(number, INTERNATIONAL);
                        final Format format = new Format(e164, international, rfc3966, national);
                        currentNumber.setFormat(format);
                        currentNumber.setNumber(format.e164());
